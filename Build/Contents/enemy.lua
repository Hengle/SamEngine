...
function enemy:update(now, delta)
	if self.state.current == "walk" then
		...
		if self.position >= 400 then
			self.state:ready()
			if Game.battle.player_attact_state ~=0 and Game.battle.move_dst > 0 then
				Game.battle.move_dst = Game.battle.move_dst - self.speed;
			end
		end
	elseif self.state.current == "before_attack" then
		if self.position < 550 then
			...
			if now - self.last_attack_time > 1000 and Game.battle.player_attact_state == 0 then
				self.state:attack()
...
function enemy:draw()
	...
	trace(self.current_animation.image.x)
	self.current_animation.image.scaleX = self.facing
	self.current_animation:draw()
	trace(self.current_animation)
end