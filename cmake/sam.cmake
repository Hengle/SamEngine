...
function (s_add_module dir)
    ...
    foreach (dep ${ARGN})
        ...
        set(module_and_depencies ${module_and_depencies} ${dep} PARENT_SCOPE)
    endforeach ()
    set(module_and_depencies ${module_and_depencies} ${name} PARENT_SCOPE)
...
function (s_add_sample dir)
    get_filename_component(full_path ${dir} ABSOLUTE)
    string(REPLACE "${PROJECT_SOURCE_DIR}/engine/samples/" "" name ${full_path})
    ...
    target_link_libraries(${name} ${module_and_depencies})
    s_set_target_folder(${name})
    set_target_properties(${name} PROPERTIES FOLDER "samples")
endfunction ()

function (s_group_tool_source name dir variable)
    file(GLOB file_list "${dir}/*")
    foreach (file ${file_list})
        if (IS_DIRECTORY ${file})
            s_group_tool_source(${file} ${variable})
        endif()
    endforeach()
    file(GLOB sources "${dir}/*.h" "${dir}/*.c" "${dir}/*.cpp")
    set(${variable} ${${variable}} ${sources} PARENT_SCOPE)
    string(REPLACE "${PROJECT_SOURCE_DIR}/engine/tools/${name}/" "" source_group ${dir})
    string(FIND ${source_group} ${PROJECT_SOURCE_DIR} index)
    if (${index} GREATER -1)
        source_group("" FILES ${sources})
    else ()
...
function (s_add_tool dir)
    get_filename_component(full_path ${dir} ABSOLUTE)
    string(REPLACE "${PROJECT_SOURCE_DIR}/engine/tools/" "" name ${full_path})
    s_group_tool_source(${name} ${full_path} tool_src)
    add_executable(${name} ${tool_src})
    target_link_libraries(${name} ${module_and_depencies})
    s_set_target_folder(${name})
